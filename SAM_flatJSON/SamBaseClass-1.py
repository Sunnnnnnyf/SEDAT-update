...
class SamBaseClass(object):
    ...
    def main(self, param = None):
        if self.cspModel=='pvsamv1':
            ...
            self.wind_module= Wind()
            self.wind_output = self.wind_module.windpower()
        ...
        if self.cspModel and self.cspModel != 'None':
        ...
        def export_wind_to_json(self):
            print('export json initiated')
            filename = self.wind + '_output' + self.timestamp + '.json'
            wind_json_outfile = f'/Users/fengxiaotong/Desktop/sedat_model_selection-main/SAM_flatJSON/{filename}'
            with open(wind_json_outfile, 'w') as outfile:
                json.dump(self.wind_output, outfile)
            print('wind model executed')
            #TODO:
            # Generate json file for wind model results
...
#            if variable == 'twet': continue
    ...
    def wind_export_to_csv(self):
        if self.wind_output:
            ...
            csv_writer.writerow(["Wind class", "IEC III"])
            csv_writer.writerow(["Hub height", "100m"])
            csv_writer.writerow(["Cutin speed", "3m/s"])
            csv_writer.writerow(["Cutout speed", "25m/s"])
            count = 0
            time_series = []
            for i in data:
