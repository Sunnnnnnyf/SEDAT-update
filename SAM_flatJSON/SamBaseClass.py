...
import SAM_flatJSON.app_config as cfg
import SAM_flatJSON.helpers
import csv
import xlwt
import os

from DesalinationModels.Crystallizer import Crystallizer  
from PSA.wind_model import wind

class SamBaseClass(object):
    """class to run design, desal, and financial models. """
    ...
    def __init__(self,
                 ...
                 timestamp = '',
                 wind_json_value_filepath= None,
                 # we need to double check the filepath for wind model
                 ):
        self.wind_gen = [0]*8760
        self.elec_gen = [0]*8760
        self.heat_gen = [0]*8760
        self.wind_lcoe = 0

        ## Set up for lambda
        ## - create folders in /tmp
        ...
        # TODO: handle path for weather file! 
        else:
        ...
        if desal_json_value_filepath:
            ...
            desal_values = self.desalination + '.json' if self.desalination else "None.json"
        ...
        if cryst_json_value_filepath:
            self.cryst_json_values = cryst_json_value_filepath
        else:
            cryst_values = self.cryst + '.json' if self.cryst else "None.json"
            self.cryst_json_values = Path(self.samPath / "defaults" /cryst_values)

        if wind_json_value_filepath:
            self.wind_json_values=wind_json_value_filepath
        else:
            wind_values = self.wind + '.json' if self.wind else "None.json"
            self.wind_json_values = Path(self.samPath / "defaults" / wind_values)

        self.project_name = 'lambda'
    ...
    def main(self, param = None):
        if self.cspModel=='pvsamv1':
            ...
            if self.desalination:
                self.desal_simulation(self.desalination, self.cryst) 
                self.cost(self.desalination, self.cryst)
            ...
            self.solar_capacity = values_json['system_capacity']
            ...
            if (not self.wind) and self.desalination:
                self.desal_simulation(self.desalination, self.cryst)
                self.cost(self.desalination, self.cryst)
            ...
            # else:
            sc_output_json_outfile =  f'/tmp/results/{filename}'
            ...
            # else:
            et_output_json_outfile =  f'/tmp/results/{filename}'
        ...
        if self.wind == "wind":
            with open(self.wind_json_values, "r") as read_file:
                wind_input = json.load(read_file)
            self.windcollector = wind(hub_height=wind_input['hub_height'],
                                      ...
                                      inflation_rate = wind_input['inflation_rate'],
                                      capex_per_kw = wind_input['capex_per_kw'],
                                      opex = wind_input['opex'],
                                      file_name=wind_input["file_name"]
            )

            self.wind_gen, self.wind_output, self.fcr, self.wind_lcoe = self.windcollector.windpower()
            self.wind_capacity = wind_input['capacity']
            ...
            #transfer to json
            filename = "wind_output"+self.timestamp + ".json"
            Wind_json_outfile = f'/tmp/results/{filename}'
            with open(Wind_json_outfile, 'w') as outfile:
                json.dump(self.wind_output, outfile)

            #transfer to csv
            filename = 'wind_output'+self.timestamp + '.json'
            csvname = 'wind_output'+ self.timestamp + '.csv'
            simulation_json_outfile = f'/tmp/results/{filename}'
            simulation_csv_outfile = f'/tmp/CSV/{csvname}'
            ...
            csv_writer.writerow(["Hub height",  wind_input["hub_height"]])
            csv_writer.writerow(["Cutin speed", wind_input["cutin_speed"]])
            csv_writer.writerow(["Cutout speed",wind_input["cutout_speed"]])
            count = 0
            time_series = []
            for i in data:
        ...
        if self.desalination:
            ...
            csvlist.append(designcsv)
            csvlist.append(simucsv)
            csvlist.append(costcsv)
            typelist.append('Desal design')
            typelist.append('Desal simulation')
            typelist.append('Cost')


        if self.cryst:
            cryst_designcsv = self.project_name + '_' +  self.cryst + '_design_output' + '_' + self.timestamp + '.csv'
            cryst_simucsv =   self.project_name + '_' +  self.cryst + '_simulation_output' + '_' + self.timestamp + '.csv'
            costcsv =   self.project_name + '_' +  self.desalination + '_cost_output' + '_' + self.timestamp + '.csv'
            csvlist.append(cryst_designcsv)
            csvlist.append(cryst_simucsv)
            typelist.append('Cryst design')
            typelist.append('Cryst simulation ')
        
        wb = xlwt.Workbook()
        for i in range(len(csvlist)):
            # if self.parametric:
            #     csvpath = self.samPath.parent / 'tmp' / 'CSV parametric_results' / csvlist[i]
            # else:
            csvpath = f'/tmp/CSV/{csvlist[i]}'
            fname = typelist[i]
        ...
        # wb.save(self.samPath.parent / 'tmp' /'XLS results' / xlxsname)
        wb.save(f'/tmp/results/{xlxsname}')
    ...
    def desal_design(self, desal, cryst):
        ...
        if desal == 'RO':
            ...
            self.elec_req = self.RO.PowerTotal # kW
            self.brine_salinity, self.brine_flow_rate = self.RO.brine_salinity, self.RO.brine_flow_rate
            self.P_req = self.RO.PowerTotal
            ...
            self.brine_salinity, self.brine_flow_rate = self.MDB.brine_salinity, self.MDB.brine_flow_rate
            ...
            self.brine_salinity, self.brine_flow_rate = self.LTMED.brine_salinity, self.LTMED.brine_flow_rate
            ...
            self.brine_salinity, self.brine_flow_rate = self.MEDTVC.brine_salinity, self.MEDTVC.brine_flow_rate
            ...
            self.elec_req = self.design_output[1]['Value'] * 1000 # kW
            self.brine_salinity, self.brine_flow_rate = self.OARO.brine_salinity, self.OARO.brine_flow_rate
        ...
        if cryst:
            from DesalinationModels.Crystallizer import Crystallizer            
            with open(self.cryst_json_values, 'r') as read_file:
                self.cryst_values_json = json.load(read_file)  
            self.cryst = Crystallizer(specified_capacity = self.cryst_values_json['specified_capacity'],
                                      desal_brine_capacity = self.desal_brine,
                                      feed_pressure = self.cryst_values_json['feed_pressure'],
                                      feed_temperature = self.cryst_values_json['feed_temperature'],
                                      crystallizer_temperature = self.cryst_values_json['crystallizer_temperature'],
                                      crystallizer_yield = self.cryst_values_json['crystallizer_yield'],
                                      Fossil_f = self.cryst_values_json['Fossil_f'],
                                      )
            self.cryst_design_output = self.cryst.design()
        ...
        if cryst:
            crystname = cryst + '_design_output' + self.timestamp + '.json'
            design_json_outfile =  f'/tmp/results/{filename}'
            with open(design_json_outfile, 'w') as outfile:
                json.dump(self.cryst_design_output, outfile)
    ...
    def desal_simulation(self, desal, cryst):
        with open(self.desal_json_values, "r") as read_file:
            self.desal_values_json = json.load(read_file)
        ...
        with open(self.cryst_json_values, 'r') as read_file:
            self.cryst_values_json = json.load(read_file)
        self.P_req = 0
        self.cryst_water_prod = 0
        self.cryst_salt_prod = 0
        ...
        if desal == 'RO':
            ...
            self.RO = RO(nominal_daily_cap_tmp = self.desal_values_json['nominal_daily_cap_tmp'], FeedC_r = self.desal_values_json['FeedC_r'],
                         ...
                         Pfp= self.desal_values_json['Pfp'],maxQf= self.desal_values_json['maxQf'])
            self.design_output = self.RO.RODesign()
            self.simu_output = self.RO.simulation(gen = [self.elec_gen[i] + self.wind_gen[i] for i in range(len(self.elec_gen))], storage = self.desal_values_json['storage_hour'])
            ...
            self.cryst_load = 0
            if cryst:
                self.Crystallizer = Crystallizer(specified_capacity = self.cryst_values_json['specified_capacity'],
                                        desal_brine_capacity = self.brine_flow_rate,
                                        specified_salinity = self.cryst_values_json['specified_salinity'],
                                        desal_brines_salinity = self.brine_salinity,
                                        feed_pressure = self.cryst_values_json['feed_pressure'],
                                        feed_temperature = self.cryst_values_json['feed_temperature'],
                                        crystallizer_temperature = self.cryst_values_json['crystallizer_temperature'],
                                        crystallizer_yield = self.cryst_values_json['crystallizer_yield'],
                ...
                self.cryst_design_output = self.Crystallizer.design() 
                self.cryst_load = self.cryst_design_output[0]['Value']
                self.cryst_simu_output = self.Crystallizer.simulation(gen = self.heat_gen, storage=self.cryst_values_json['storage_hour'], desal_load=self.P_req)
                self.cryst_water_prod = self.cryst_simu_output[5]['Value']
                self.cryst_salt_prod = self.cryst_simu_output[6]['Value']
            self.simu_output = self.MDB.simulation(gen = self.heat_gen, storage = self.desal_values_json['storage_hour'], cryst_load=self.cryst_load)
            ...
            if cryst:
                ...
                self.cryst_water_prod = self.cryst_simu_output[5]['Value']
                self.cryst_salt_prod = self.cryst_simu_output[6]['Value']               
            self.simu_output = self.LTMED.simulation(gen = self.heat_gen, storage = self.desal_values_json['storage_hour'], cryst_load=self.cryst_load)
            ...
            if cryst:
                ...
                self.cryst_water_prod = self.cryst_simu_output[5]['Value']
                self.cryst_salt_prod = self.cryst_simu_output[6]['Value']       
            self.simu_output = self.MEDTVC.simulation(gen = self.heat_gen, storage = self.desal_values_json['storage_hour'], cryst_load=self.cryst_load)
            ...
            if self.cspModel=='pvsamv1' or self.cspModel=='pvwattsv7':
                ...
                self.simu_output = self.RO_FO.simulation(elec_gen = self.heat_gen, thermal_gen = self.heat_gen, solar_type = 'csp', storage = 0)
        ...
        if desal:
            ...
            # else:
            design_json_outfile =  f'/tmp/results/{filename}'
            design_csv_outfile =  f'/tmp/CSV/{csvname}'
            with open(design_json_outfile, 'w') as outfile:
                json.dump(self.design_output, outfile)
        ...
        if cryst:
            filename = cryst + '_design_output' + self.timestamp + '.json'
            csvname  = self.project_name + '_' + cryst + '_design_output' + '_' + self.timestamp + '.csv'
            # if self.parametric:
            #     design_json_outfile = self.samPath / 'parametric_results' / filename 
            #     design_csv_outfile = self.samPath / 'CSV parametric_results' / csvname 
            # else:
            ...
            with open(map_json, "r") as read_file:
                map_data = json.load(read_file)   
            csv_writer.writerow(["Location", map_data['county'] + '  ' + map_data['state']])
            csv_writer.writerow(["Crystallization technology", self.cryst])
            csv_writer.writerow([])        
        ...
        # Write simulation output to csv
            filename = cryst + '_simulation_output' + self.timestamp + '.json'
            csvname = self.project_name + '_' + cryst + '_simulation_output' + '_' + self.timestamp + '.csv'  
            ...
            simulation_csv_outfile = f'/tmp/CSV/{csvname}'
            with open(simulation_json_outfile, 'w') as outfile:
                json.dump(self.cryst_simu_output, outfile)
    ...
    def cost(self, desal, cryst):
        ...
        if desal == 'RO':
            ...
            self.LCOW = RO_cost(Capacity = Capacity, Prod = sum(self.simu_output[0]['Value']), fuel_usage = self.simu_output[7]['Value'], wind_capacity = self.wind_capacity, solar_capacity = self.solar_capacity,
            ...
            self.LCOW = MDB_cost(crystallizer = cryst, cryst_water_prod = self.cryst_water_prod, cryst_salt_prod = self.cryst_salt_prod,
                                 Capacity = self.desal_values_json['Capacity'], Prod = self.simu_output[4]['Value'], fuel_usage = self.simu_output[7]['Value'], Area = self.MDB.Area,
            ...
            self.LCOW = LTMED_cost(crystallizer = cryst, cryst_water_prod = self.cryst_water_prod, cryst_salt_prod = self.cryst_salt_prod,
                                   f_HEX = self.cost_values_json['f_HEX'], downtime = self.cost_values_json['downtime'],
            ...
            self.LCOW = MEDTVC_cost(crystallizer = cryst, cryst_water_prod = self.cryst_water_prod, cryst_salt_prod = self.cryst_salt_prod,
                                    f_HEX = self.cost_values_json['f_HEX'], downtime = self.cost_values_json['downtime'],
        ...
        # else:
        cost_json_outfile =f'/tmp/results/{filename}' 
        cost_csv_outfile = f'/tmp/CSV/{csvname}'              
...
#                json_files.append(Path(self.samPath / "models" / "inputs" / 'lcoefcr_inputs.json'))
        ...
        for json_file in json_files:
            ...
            for variable in all_variables:
                ...
                elif variable['Name'] == 'capital_cost':
                    varValue = values_json['capital_cost'] * values_json['system_capacity']


                elif variable['Name'] == 'fixed_operating_cost':
                    varValue = values_json['fixed_operating_cost'] * values_json['system_capacity']
...
#                    print(variable['Name'], varValue)
            ...
            #variable.valu
            # inputfile = self.samPath.parent /'tmp' / 'results' /'inputdata.json'
            inputfile = '/tmp/results/inputdata.json'
...
#        print(Ms)
    ...
    def print_collector_params(self, collector_outputs):
        ...
        # else:
        json_outfile = f'/tmp/results{filename}'
        csv_outfile = f'/tmp/CSV/{csvname}'
